...
blocks=min(len(xpref[:,0])//N+1, len(mTbarkquantflattenedout[:])//nfilts); print("blocks=", blocks) #min number of signal blocks in the files  
for chan in range(channels): #loop over channels:
   ...
   mTbarkquant=np.reshape(mTbarkquantflattenedout[0:nfilts*(blocks-1)], (nfilts,-1),order='F')
