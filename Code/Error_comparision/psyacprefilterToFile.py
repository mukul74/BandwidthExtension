...
# # x = x/np.max(np.abs(x))
# x = x + np.max(np.abs(x))*noise_component*np.random.normal(loc=0.0, scale=1.0,size=x.shape)
...
preffile=name+ '_' + quality_str + '_'  +  'pref.wav'
print("Prefiltered file:", preffile)
maskingthresholdfile=name+ '_' + quality_str + '_' +'mT.wav'
