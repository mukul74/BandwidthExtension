...
def MDCT_psayac_quant_enc(x,fs,fb,N, nfilts=64,quality=100):
   ...
   for m in range(M): #M: number of blocks
     ...
     mXbark=mapping2bark(np.abs(y[:,m]),W,nfft)
...
if __name__ == '__main__':
   ...
   #x=x[:,0]
   fs, x= wav.read('C://D_drive//MT//Codes//Python-Audio-Coder//fantasy-orchestra.wav')
   x=x[:,0]
   ...
   yq, y, mTbarkquant = MDCT_psayac_quant_enc(x,fs,fb,N, nfilts,quality=100)
