...
def main():
    ...
    #     Wn_output = Wn_output[None, None, :]
    #     Wn_input = Wn_input.repeat(torch.tensor(32,dtype=torch.int8), 1, 1).to("cuda")
    #     Wn_output = Wn_output.repeat(torch.tensor(32,dtype=torch.int8), 1, 1).to("cuda")
    ...
    #             batch_target = batch_target*Wn_output
    #
    #             Weighted_matrix = []
    #             #### LPC Calculation and Modified Weighted Perceptual Loss ####
    #             sr = target_samplerate
    #             gamma_1 = 0.92
    #             gamma_2 = 0.60
    #             Coef_num = 12
    #             for bt in range(batch_target.shape[0]):
    #                 audio_np = batch_target[bt,0,:].detach().cpu().numpy()
    ...
    #                 Weighted_matrix.append(torch.from_numpy(np.abs(freqz_amp_values)).to('cuda'))
    #
    #             opt.zero_grad()
    #             predicted_target = net((batch_inputs,hidden_reset))
    #
    #             STFT_Original = torch.stft(batch_target.squeeze(), n_fft=2046, return_complex=True)
    #             STFT_Reconstructed = torch.stft(predicted_target.squeeze(), n_fft=2046, return_complex=True)
    #
    #
    #             loss = weighted_mse_loss(STFT_Original,STFT_Reconstructed,torch.stack(Weighted_matrix),criterion)
    #             Weighted_matrix.clear()
    #
    #             Tloss += loss.item()
    #
